<template>
    <div class="space-y-6">
        ...
        <Card>
            <CardContent class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <BaseFormField v-model="filters.search" placeholder="Buscar categorias..."
                    ...
                    <div class="flex space-x-2">
                        <Button variant="outline" @click="toggleSortOrder" class="flex-1">
                            <component :is="filters.sortOrder === 'asc' ? ArrowUp : ArrowDown" class="w-4 h-4 mr-2" />
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <Card v-for="category in categoriesStore.categories" :key="category.id"
                class="hover:shadow-md transition-shadow cursor-pointer group" @click="editCategory(category)">
                <CardContent class="p-6">
                    <div class="flex items-start justify-between mb-4">
                        <div class="w-12 h-12 rounded-lg flex items-center justify-center"
                            :style="{ backgroundColor: category.color + '20' }">
                            <Folder class="w-6 h-6" :style="{ color: category.color }" />
                        </div>
                        <div class="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Button variant="ghost" size="sm" @click.stop="editCategory(category)">
                                <Edit2 class="w-4 h-4" />
                            </Button>
                            <Button variant="ghost" size="sm" @click.stop="confirmDeleteCategory(category)"
        ...
        <!-- Pagination -->
        <div v-if="categoriesStore.pagination.totalPages > 1" class="flex items-center justify-between">
            <div class="text-sm text-gray-700">
                Mostrando {{ (categoriesStore.pagination.page - 1) * categoriesStore.pagination.limit + 1 }} a
                {{ Math.min(categoriesStore.pagination.page * categoriesStore.pagination.limit,
                    categoriesStore.pagination.total) }}
                de {{ categoriesStore.pagination.total }} resultados
            </div>
            <div class="flex space-x-2">
                <Button variant="outline" size="sm" :disabled="categoriesStore.pagination.page === 1"
    ...
    <CategoryFormDialog :is-open="showCategoryDialog" :category="selectedCategory" @close="closeCategoryDialog"
        @saved="handleCategorySaved" />

    <!-- Confirm Delete Dialog -->
    <ConfirmDialog :is-open="showDeleteDialog" type="danger" title="Excluir Categoria"
        ...
        confirm-text="Excluir" :loading="deleteLoading" @confirm="handleDeleteCategory" @cancel="closeDeleteDialog" />
</template>
